...
function renderQuestion(){
  ...
  q.opts.forEach(function(o,i){
    ...
    btn.innerHTML='<span class="opt-letter">'+String.fromCharCode(65+i)+'</span>'+escHtml(o);
    btn.onclick=function(){handleSelect(i);};
...
function handleSelect(oi){
  ...
  const q=pool[idx];
  results[idx]={qId:q.id,chosen:oi,correct:q.ans};
  $("opts").querySelectorAll(".opt-btn").forEach(function(b,i){
    b.classList.add("locked");
    if(i===q.ans)b.classList.add("correct");
    else if(i===oi&&oi!==q.ans)b.classList.add("wrong");
  });
  $("fb-text").textContent=oi===q.ans?"✅ 正解！":"❌ 正解は："+q.opts[q.ans];
  ...
  if(wrong.length>0){
    ...
    wrong.forEach(r=>{
      ...
      card.innerHTML='<span class="badge" style="background:'+BADGE_BG[q.cat]+';color:'+BADGE_FG[q.cat]+'">'+CAT_LABEL[q.cat]+'</span>'
        ...
        +'<p style="font-size:13px;color:var(--text2)">あなたの回答：<span style="color:'+c.accent+'">'+escHtml(q.opts[r.chosen])+'</span></p>'
        +'<p style="font-size:13px;color:var(--text2);margin-top:2px">正解：<span style="color:'+c.accent+';font-weight:700">'+escHtml(q.opts[r.correct])+'</span></p>'
